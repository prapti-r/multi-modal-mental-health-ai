"""
ml/speech_emotion.py
─────────────────────
Speech processing: Whisper transcription + Wav2Vec2 voice emotion.

Pre-trained:  openai/whisper-base            (speech-to-text)
Pre-trained:  ehcalabres/wav2vec2-lg-xlsr-en-speech-emotion-recognition
Fine-tuned on RAVDESS + IEMOCAP via ml/training/train_speech_emotion.py

Wav2Vec2 output labels (from the HuggingFace model card):
  angry, calm, disgust, fearful, happy, neutral, sad, surprised

These are collapsed to our 5-label taxonomy:
  anger | anxiety | positive | neutral | sadness

Librosa features are still extracted and stored in AiAnalysisResult.voice_features
for the Late Fusion model — Wav2Vec2 handles the emotion classification.

Public interface:
  load_models(whisper_size)        → None
  transcribe(audio_bytes, mime)    → str
  extract_voice_features(bytes)    → VoiceAnalysisResult
"""
...
_WHISPER_MODEL    = None
_VOICE_CLASSIFIER = None   # HuggingFace pipeline (not a raw torch model)
...
# Emotions that contribute to PRD §7.1 physiological distress trigger (> 85%)
...
async def transcribe(audio_bytes: bytes, mime_type: str) -> str:
    ...
    def _run() -> str:
        ...
        try:
            ...
            os.unlink(tmp)   # PRD §5 — delete immediately
...
def _wav2vec2_infer(audio_bytes: bytes) -> tuple[str, float, dict[str, float]]:
    ...
    try:
        ...
        # via a numpy array. We use soundfile to decode first.
        import numpy as np
